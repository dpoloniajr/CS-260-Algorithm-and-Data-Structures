...
int partition(vector<Bid> &bids, int begin, int end) {
	...
	while (!done) {
		...
		if (low >= high) {
			...
			//swap the low and the high bids using built-in vector method
...
void quickSort(vector<Bid> &bids, int begin, int end) {
	...
	//parition bids into low and high
...
void selectionSort(vector<Bid> &bids) {
	...
	//pos is the position within the bids that marks the sorted/unsorted
	for (unsigned pos = 0; pos < bids.size(); ++pos) {
		min = pos;

		// j is the position within the bids that marks the comparison against the pos variable
		...
		//If the bid title doesnt match the pos variable then swap positions with the j variable
		if (min != pos) {
...
int main(int argc, char *argv[]) {
	...
	while (choice != 9) {
		...
		case 3:
			// Initialize a timer variable before sorting the bids
			ticks = clock();
		...
		case 4:
			// Initialize a timer variable before Quick-sorting the bids
			ticks = clock();
